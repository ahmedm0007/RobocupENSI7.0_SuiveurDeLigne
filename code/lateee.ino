...
// PID functions
void  PID(double KP,long KI,double KD,double speed) 
{ 
  switch(ir3())
  {
    case 0 :
  ...
  BleftSpeed = speed - Bcorrection; //45
  BrightSpeed = speed + Bcorrection;//45
...
void Blackfollower1()
{
  PID(70,0.8,20,100); //order(kp,ki,kd)
}
...
digitalWrite(LED,LOW);
  ...
  { 
    ...
    {  
      ...
      case bfollower1:  //wael's start
      { digitalWrite(LED,LOW);
          Blackfollower1();
          readIR();  
        if(LL_Value==1 && RR_Value==1 )
      ...
      while(calcul_compteur()<3)
       {
       rotate_motor_forward(RF,RB,RP,25);
       rotate_motor_forward(LF,LB,LP,80);
       //delay(300);
       Pre_LL_Value=LL_Value;
       digitalWrite(LED,HIGH);
        readIR();
      ...
      ROBOT_CurrentState_en = bfollower2;
      ...
      { digitalWrite(LED,LOW);
        ...
        //(currentTime>=10000)
            ROBOT_CurrentState_en = bHexagone;
      }break;
    ...
    if (((R_Value == 0) && (RR_Value == 0) && (M_Value == 1)) || (millis() - cornerStartTime > 720))  // Example timeout after 2 seconds
    {
      ...
      */
      ...
      {digitalWrite(LED,LOW);
        Blackfollower2();
        if (L_Value==1 && R_Value==1 && M_Value==1)
          ROBOT_CurrentState_en = stop;
      }
     ...
     case bHexagone:
      { digitalWrite(LED,HIGH);
       ...
       while(M_Value!=1 && L_Value!=0 && LL_Value!=0 && R_Value!=0 && RR_Value!=0)
        {motor_forward();}
           ROBOT_CurrentState_en =bfollower9;
        }break;

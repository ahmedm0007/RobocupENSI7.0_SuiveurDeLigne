...
void bot_idle1fct()//first starting function
{
      ...
      rotate_motor_forward(RF,RB,RP,60);
      rotate_motor_forward(LF,LB,LP,40);
  ...
  { 
    switch (ROBOT_CurrentState_en)
    {
      ...
      { digitalWrite(LED,LOW);
        Blackfollower1();
        readIR();
        if (R_Value==1 && M_Value==1 && L_Value==1 ){
     ...
     case fHexagone:
      ...
      //motor_stop();
       ...
       { 
        rotate_motor_backward(RF, RB, RP, 0);
...
*/
    ...
    //Serial.println("this is the rightspeed"+BrightSpeed);
    //Serial.println("this is the leftspeed"+BleftSpeed);
   /*R_Value = digitalRead(R);  
    delay(1000);
    Serial.print("this is R ");
